...
@interface ContractImageTableViewCell()<AddImageViewDelegate>
{
...
-(void)addImageViewAddImageButtonClick{
    //添加图片
    if (self.delegate && [self.delegate respondsToSelector:@selector(addContractImage)]) {
        [self.delegate addContractImage];
    }
}
-(void)deleteImageViewDelImageButtonClick:(NSInteger)index{
    //删除图片
    if (self.delegate && [self.delegate respondsToSelector:@selector(deleteContractImageWithIndex:)]) {
        [self.delegate deleteContractImageWithIndex:index];
    }
}
